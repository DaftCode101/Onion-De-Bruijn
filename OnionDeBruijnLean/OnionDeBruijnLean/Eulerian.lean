...
--------------------------------------------------------------------------------
-- 1. The Fundamental Root Graph G(n,k)
--------------------------------------------------------------------------------

-- Def 3.1: The fundamental root graph G(n,k)
def G (n k : ℕ) (u v : Word (n - 1) k) : Prop :=
  DB u v ∧ (HasKMinusOne u ∨ HasKMinusOne v)
...
lemma is_in_edge {m k : ℕ} (c : Fin k) (v : Word m k) : DB (prepend_char c v) v := by
  intro i
  simp [prepend_char]

lemma is_out_edge {m k : ℕ} (v : Word m k) (c : Fin k) : DB v (append_char v c) := by
  intro i
  have h : (i : ℕ) ≠ m - 1 := by omega
  simp [append_char, h]

noncomputable def out_edges_equiv (n k : ℕ) (hn : 2 ≤ n) (v : Word (n - 1) k) : 
  {u : Word (n - 1) k // DB v u} ≃ Fin k where
...
noncomputable def in_edges_equiv (n k : ℕ) (hn : 2 ≤ n) (v : Word (n - 1) k) : 
  {u : Word (n - 1) k // DB u v} ≃ Fin k where
...
noncomputable def case2_out_equiv (n k : ℕ) (hn : 2 ≤ n) (v : Word (n - 1) k) 
    (h : ¬HasKMinusOne v) :
  {u : Word (n - 1) k // DB v u ∧ HasKMinusOne u} ≃ {c : Fin k // (c : ℕ) = k - 1} where
...
noncomputable def case2_in_equiv (n k : ℕ) (hn : 2 ≤ n) (v : Word (n - 1) k) 
    (h : ¬HasKMinusOne v) :
  {u : Word (n - 1) k // DB u v ∧ HasKMinusOne u} ≃ {c : Fin k // (c : ℕ) = k - 1} where
...
lemma out_degree_has_k_minus_one {n k : ℕ} (hn : 2 ≤ n) (_hk : 1 ≤ k) 
    [Fintype (Word (n - 1) k)] (v : Word (n - 1) k) (h : HasKMinusOne v) :
  OutDegree (G n k) v = k := by
  dsimp [OutDegree]
  have eq1 : filter (fun u => G n k v u) univ = filter (fun u => DB v u) univ := by
    apply filter_congr; intro u _; dsimp [G]; simp [h]
  rw [eq1]
  have eq2 : (filter (fun u => DB v u) univ).card = Fintype.card {u // DB v u} := by
    exact (Fintype.card_subtype _).symm
  rw [eq2]
  have eq3 : Fintype.card {u // DB v u} = Fintype.card (Fin k) := by
    exact Fintype.card_congr (out_edges_equiv n k hn v)
  rw [eq3]
  exact Fintype.card_fin k

lemma out_degree_not_has_k_minus_one {n k : ℕ} (hn : 2 ≤ n) (hk : 1 ≤ k) 
    [Fintype (Word (n - 1) k)] (v : Word (n - 1) k) (h : ¬HasKMinusOne v) :
  OutDegree (G n k) v = 1 := by
  dsimp [OutDegree]
  have eq1 : filter (fun u => G n k v u) univ = 
             filter (fun u => DB v u ∧ HasKMinusOne u) univ := by
    apply filter_congr; intro u _; dsimp [G]; simp [h]
  rw [eq1]
  have eq2 : (filter (fun u => DB v u ∧ HasKMinusOne u) univ).card = 
             Fintype.card {u // DB v u ∧ HasKMinusOne u} := by
    exact (Fintype.card_subtype _).symm
  rw [eq2]
  have eq3 : Fintype.card {u // DB v u ∧ HasKMinusOne u} = 
...
--------------------------------------------------------------------------------
-- 2. In/Out Degree Bijections and Eulerian Properties (Lemma 3.2)
--------------------------------------------------------------------------------
...
lemma root_graph_eulerian (n k : ℕ) (hn : 2 ≤ n) (hk : 1 ≤ k) [Fintype (Word (n - 1) k)] : 
  ...
  · -- Case 1
    have h_out : OutDegree (G n k) v = k := by
      dsimp [OutDegree]
      have eq1 : filter (fun u => G n k v u) univ = filter (fun u => DB v u) univ := by
        apply filter_congr
        intro u _
      ...
      rw [eq3]
      exact Fintype.card_fin k
    have h_in : InDegree (G n k) v = k := by
      dsimp [InDegree]
      have eq1 : filter (fun u => G n k u v) univ = filter (fun u => DB u v) univ := by
        apply filter_congr
        intro u _
        dsimp [G]
        simp [h]
      rw [eq1]
      have eq2 : (filter (fun u => DB u v) univ).card = Fintype.card {u // DB u v} := by
        exact (Fintype.card_subtype _).symm
      rw [eq2]
      have eq3 : Fintype.card {u // DB u v} = Fintype.card (Fin k) := by
        exact Fintype.card_congr (in_edges_equiv n k hn v)
      rw [eq3]
      exact Fintype.card_fin k
    rw [h_out, h_in]
  · -- Case 2
    have h_out : OutDegree (G n k) v = 1 := by
      dsimp [OutDegree]
      have eq1 : filter (fun u => G n k v u) univ = 
                 filter (fun u => DB v u ∧ HasKMinusOne u) univ := by
        apply filter_congr
      ...
      have eq4 : Fintype.card {c : Fin k // (c : ℕ) = k - 1} = 1 := by
        ...
        rw [Fintype.card_congr e, Fintype.card_unit]
      exact eq4
    have h_in : InDegree (G n k) v = 1 := by
      dsimp [InDegree]
      have eq1 : filter (fun u => G n k u v) univ = 
                 filter (fun u => DB u v ∧ HasKMinusOne u) univ := by
        apply filter_congr
        intro u _
        dsimp [G]
        simp [h]
      rw [eq1]
      have eq2 : (filter (fun u => DB u v ∧ HasKMinusOne u) univ).card = 
                 Fintype.card {u // DB u v ∧ HasKMinusOne u} := by
        exact (Fintype.card_subtype _).symm
      rw [eq2]
      have eq3 : Fintype.card {u // DB u v ∧ HasKMinusOne u} = 
                 Fintype.card {c : Fin k // (c : ℕ) = k - 1} := by
...
--------------------------------------------------------------------------------
-- 4. Word Splitting and Gluing Equivalences
--------------------------------------------------------------------------------

def glue {n k : ℕ} (hn : 2 ≤ n) (u v : Word (n - 1) k) (_h : DB u v) : Word n k :=
...
lemma split_is_edge {n k : ℕ} (hn : 2 ≤ n) (w : Word n k) : 
  DB (split_left hn w) (split_right hn w) := by
  intro i
  dsimp [split_left, split_right]

lemma glue_split_left {n k : ℕ} (hn : 2 ≤ n) (u v : Word (n - 1) k) (h : DB u v) :
...
lemma glue_split_right {n k : ℕ} (hn : 2 ≤ n) (u v : Word (n - 1) k) (h : DB u v) :
...
lemma has_k_minus_one_glue {n k : ℕ} (hn : 2 ≤ n) (u v : Word (n - 1) k) (h : DB u v) :
...
def layer_equiv_root_edges (n k : ℕ) (hn : 2 ≤ n) : 
  {e : Word (n - 1) k × Word (n - 1) k // G n k e.1 e.2} ≃ LayerVertex n k where
...
lemma db_edge_iff_split {n k : ℕ} (hn : 2 ≤ n) (w1 w2 : Word n k) :
  DB w1 w2 ↔ split_right hn w1 = split_left hn w2 := by
  constructor
  · intro h
    funext i
...
--------------------------------------------------------------------------------
-- 5. L(G(n,k)) ≅ Lay(n,k) (Lemma 3.3)
--------------------------------------------------------------------------------

theorem layer_is_line_graph (n k : ℕ) (hn : 2 ≤ n) :
  RelIsomorphic (LineGraph (G n k)) (Lay n k) := by
