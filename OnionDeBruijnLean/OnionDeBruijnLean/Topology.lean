...
theorem lay_degree_imbalance (n k : ℕ) (hn : 2 ≤ n) (hk : 2 ≤ k) : 
  ...
  · let e_out_word : Word n k := fun i => 
    ...
    constructor
    · intro i
      dsimp [v, v_word, e_out, e_out_word, Lay, DBEdge]
  ...
  · let in_edge_word (c : Fin k) : Word n k := fun i => 
    ...
    · intro u
      ...
      · intro hu
        ...
        apply funext
        intro i
        dsimp [in_edges, in_edge_word, Lay, DBEdge] at *
        by_cases h0 : i.val = 0
      ...
      · intro ⟨c, hc⟩
        rw [←hc]
        intro i
        dsimp [in_edges, in_edge_word, v, v_word, Lay, DBEdge]
        by_cases h0 : i.val = 0
...
-- Theorem 2.2 Heuchenne's Condition
